...
class ImageSharing(commands.Cog):
    def __init__(self, bot):
        self.bot = bot
    @cog_slash(brief='Shows the Guidelines', description='Shows the Guidelines', aliases=["rule", "rules"])
    ...
    @cog_slash(brief='Sends an invite to join the official Img.Share server', description='Sends an invite to join the official Img.Share server')
    async def server(self, ctx):
        await ctx.send(f'https://discord.gg/MrtBFFQk3k')
        print(f'someone just sent and invite to join the server!')

    @cog_slash(brief='Sends an invite to the bot', description='Sends an invite to the bot')
    async def invite(self, ctx):
        await ctx.send(f'https://www.thediamondk.com/bot.html')
        print(f'someone just invited the bot!')

    @cog_slash(brief='Pings the bot', description='Pings the bot')
    async def ping(self, ctx):
        await ctx.send(f'`{round(self.bot.latency * 1000)} ms`')
        print(f'the bot was pinged' f' ({round(self.bot.latency * 1000)} ms)')

    @cog_slash(brief='Gets a random Meme', description='Gets a random Meme', aliases=["randomeme", "rdmmeme", "rdmeme"])
    ...
    @cog_slash(brief='Posts A Meme to the Meme folder', description='Posts A Meme to the Meme folder')
    ...
    @cog_slash(brief='Posts A Meme Video to the Meme folder', description='Posts A Meme Video to the Meme folder')
    ...
    @cog_slash(brief='Gets a random pet', description='Gets a random pet', aliases=["rdmpet", "rdpet"])
    ...
    @cog_slash(brief='Posts A pet to the pet folder', description='Posts A pet to the pet folder')
    ...
    @cog_slash(brief='Gets a certain meme', description='Gets a certain meme', aliases=["ctmeme", "ctm"])
    ...
    @cog_slash(brief='Gets a certan pet', description='Gets a certan pet', aliases=["ctpet", "ctp"])
    ...
    @cog_slash(brief='Shows all of the memes', description='Shows all of the memes')
    ...
    @cog_slash(brief='Shows all of the pets', description='Shows all of the pets')
    ...
    @cog_slash(brief='admin command', description='admin command')
