...
class Get(commands.Cog):
    ...
    async def randommeme(self, ctx):
        with sentry_sdk.start_span(op='randommeme', description='Gets a random meme') as span:
    ...
    async def randompet(self, ctx):
        with sentry_sdk.start_span(op='randompet', description='Gets a random pet') as span:
            bot_message = await ctx.send("<a:loading:851251570971770920> sending...")
...
def setup(bot):
>>>>>>> 2b6bb246cc492e4e4e0af264f47139e8ed5ede9d
    bot.add_cog(Get(bot))